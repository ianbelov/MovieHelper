...
const val MOVIE_QUERY = "saved_movies"
...
class MovieDetailsPresenter @Inject constructor(
    ...
    fun getProviders(id: String?) {
        id?.let {
            searchRepository.searchProviders("tmdb", "movie/"+it).subscribeOn(schedulers.io())
                .observeOn(schedulers.ui())
                .subscribeBy(onSuccess = {
    ...
    fun updateCatchedPokemons(movies: List<String>, isAdded: Boolean): Single<Boolean> =
        Single.create {
            db.reference.child(auth.currentUser!!.uid).child(MOVIE_QUERY).setValue(movies)
    ...
    fun getCatchedPokemonsIds(): Single<MutableList<String>> {
        if (auth.currentUser != null) {
            return Single.create {
                db.reference.child(auth.currentUser!!.uid).child(MOVIE_QUERY)
                    .get()
